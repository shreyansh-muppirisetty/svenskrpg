    ...
    try {
      if (id === "class") {
        const raw = await gemini(key, [{
          role: "user",
          text: `Simulate a Swedish WhatsApp class group chat (Klass 8B, 15-year-olds, 22 members). Generate 5-7 messages the user missed while offline. Characters: ${CLASS_CHARS}. Rules: very short messages (3-12 words each), no emoji spam, mix Swedish/English, realistic teen topics, stay true to each personality. Viggo only appears if there is drama. Occasionally one is an image or voice note. For "audio" messages, "text" MUST contain the spoken Swedish words of the voice note (it is played aloud, never shown). Return ONLY a JSON array, no other text: [{"name":"Maja","text":"omg guys","type":"text"},{"name":"Hugo","text":"","type":"image","imageDesc":"training today"},{"name":"Emma","text":"hej alla, glöm inte provet imorgon","type":"audio","duration":"0:11"}]`
        }], undefined, 500);
